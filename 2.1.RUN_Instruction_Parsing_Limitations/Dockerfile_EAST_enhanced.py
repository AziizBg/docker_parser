...
def handle_run(y, x):
    """
    Enhanced RUN instruction parser with support for complex command separators
    ...
    Returns:
        list: Structured representation of RUN command with command segments and separators
    """
    # Split the command into segments
    segments = split_run_commands(y)
    ...
    for i, segment in enumerate(segments):
        # Parse command with variable support
        parsed_command = parse_value_with_variables(segment['command'])
        
        # Create command segment structure
        segment_structure = ['command_segment', ['command', parsed_command]]
        
        # Add separator information if present
        if segment['separator']:
            segment_structure.append(['separator', [segment['separator']]])
            segment_structure.append(['type', [segment['type']]])
        
        result.append(segment_structure)
